...
| 当前机制 | 质押本金代币，每 12 小时结算 0.1% – 1.0%，三种奖励一律以 XO 发放 | 核心价值代币，私募唯一渠道，只能卖不能买，提取即销毁 |
...
| EXON 当前做什么 | 怎么发生 |
| :-- | :-- |
| 燃料买入 | 每笔入金 28% 按当时 1 U 等值买入，存入燃料钱包，只能销毁 |
| 提取销毁 | 提取收益时按到账方式从燃料钱包销毁：立即 30%、30 天 20%、60 天 10% |
...
每一笔质押入金开单即分成两份：28% 按当时 1 U 等值买入 EXON 存入燃料钱包，只能销毁；其余兑换 XO 进入质押，每 12 小时结算。质押越多，买入越多。
...
| :-- | :-- |
| ① 入金 | 10,000 U |
| ② 拆分 | 2,800 U 按 1 U 等值买入 2,800 枚 EXON 存入燃料钱包；其余兑换 XO 进入质押 |
| ③ 结算 | 每 12 小时一次，单次 0.1% – 1.0%，一天 20 – 200 U，收益以 XO 到账 |
| ④ 加成 | 选 540 天档 +50%，一天 30 – 300 U |
| ⑤ 提取 | 立即到账烧 30%、30 天到账烧 20%、60 天到账烧 10%，从燃料钱包销毁 EXON |

同样 10,000 U，选不同期限，只算原订单，期满拿到的差得很远：

| 期限 | 期限加成 | 加成后每天产出 | 期满本金 + 收益 | 为本金的倍数 |
| :-- | --: | --: | --: | --: |
| 30 天 | 无 | 20 – 200 U | 10,600 – 16,000 U | 1.1 – 1.6 倍 |
| 90 天 | +10% | 22 – 220 U | 11,980 – 29,800 U | 1.2 – 3.0 倍 |
| 360 天 | +30% | 26 – 260 U | 19,360 – 103,600 U | 1.9 – 10.4 倍 |
| 540 天 | +50% | 30 – 300 U | 26,200 – 172,000 U | 2.6 – 17.2 倍 |

收益满 100 U 就能开一张新订单，新订单沿用原订单的期限加成、同样计息，一直滚到原订单期满。按单次 0.1% 计，540 天档期满 **50,298 U，是本金的 5.0 倍**；只算原订单是 2.6 倍——时间站在质押者这一边。

### 时间是收益的变量 <a href="#s6-2" id="s6-2"></a>

质押进入选定的期限，期限越长加成越高，只看期限不看金额。单次 0.1% – 1.0% 乘以加成，每 12 小时结算一次：

![](.gitbook/assets/onepage-05-term-ladder.svg)

| 期限 | 加成 | 质押 10,000 U · 每天 | 期满 | 累计锁定 |
| :-- | --: | --: | :-- | --: |
| 30 天 | 基础 | 20 – 200 U | 第 31 天退出窗口 | 1,200 天 |
...
30 天档第 31 天是退出窗口，本金加收益一起领，不收违约金；错过自动续期 90 → 180 → 360 → 540 天，累计锁定 1,200 天。直接开 540 天档，加成 +50%，累计锁定只有 540 天——**锁得最短，拿得最多**。
...
提取收益有三种到账方式，三档之间唯一变化的变量是**你愿意等多久**：

| 到账方式 | 燃料钱包销毁 | 提取 10,000 U 销毁（按 1.0 U 计） |
| :-- | --: | --: |
| 立即到账 | 30% | 3,000 枚 |
| 30 天线性到账 | 20% | 2,000 枚 |
| 60 天线性到账 | 10% | 1,000 枚 |
...
质押到 5,000 万 U 起，一年烧掉的就超过一年释放出来的；到 1 亿 U，一天最多烧 60 万枚，是日释放的 3.3 倍。质押越多，买入越多；提取越多，烧得越多；流通盘只会越来越小。
...
|  | 内容 |
| :-- | :-- |
| **已确认** | 双层架构与统一账户 · 28% 燃料钱包 · 每 12 小时 0.1% – 1.0% 结算与期限加成 · 第 31 天退出窗口 · 私募 0.1 U 上线 1.0 U · 1,095 天线性释放 · 三档到账与燃料销毁 · 20 代推广奖励与 V1 – V12 领导奖金 |
