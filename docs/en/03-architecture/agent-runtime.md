...
Applied to an approved Staking Platform order, the runtime **validates** the published fields rather than improvising them. It verifies the 72% XO staking/PV base, the 28% EXON buy into Treasury Liquidity and the matching `F = 0.28 × P` EXON balance. The fuel stays in the user's account. A failed balance check stops the order — it does not quietly authorize selling something else to cover it.
