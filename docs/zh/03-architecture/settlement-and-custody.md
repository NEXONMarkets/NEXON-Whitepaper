...
路径在两种托管模型之间跨越时，**边界上要重新解释一次**。链上转账可能不可逆；交易所订单按场所规则终局；卡交易在发卡条款下可能支持拒付；酒店预订可能只在供应商截止时间之前可取消。「回滚」这个词在这四种情况里根本不是同一个意思。
...
| Treasury 记录 | 通过 28% 建仓买入、注入 Treasury Liquidity 的 EXON |
...
`W` 是待领收益，`b` 是选定的销毁率：T+0 为 30%，30D 为 15%，60D 为 0%。系统必须在完成赎回之前核验等值 EXON 的销毁并留下永久销毁记录。**这条机制属于赎回选择本身**——它不是 PayFi 的路径费、不是卡片费、也不是商城的手续费。

`F = 0.28 × P` 的燃料余额同样不是转给 Treasury 的托管。它是一次资格校验，资产仍在用户账户里。账本必须把 **Treasury 持有的 B** 和 **用户被校验的 F** 分开记。
...
Treasury 余额、代币销毁和自动重试都不构成用户层面的恢复保证。**恢复取决于那一段的原生系统、可用流动性、合约行为、责任运营方和适用条款。**
