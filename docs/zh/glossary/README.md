...
| 结算周期 | Epoch | 12 小时的收益与释放间隔，每天两次 |
| 燃料 | Fuel | 入金的 28% 按当时 1 U 等值买入的 EXON；显示在账户里，只能在提取静态收益时销毁，不能提现、不能转出 |
| 贡献值 | X Points | 各等级每日按标准消耗的积分（V1、V2 为 0），完成当日消耗方可领取当日平台奖励；1 点 = 价值 10 美元的 EXON，每晋升 1 级赠送 30 点，新晋级 3 个月减免期按 50% 消耗 |
...
| 质押本金代币 | Staking Principal Token | XO 的当前机制角色：本金在它上面计息，静态收益以它发放，在 NEX 自由交易 |
| 核心价值代币 | Core Value Token | EXON 的当前机制角色：私募认购或做动态赚取、只能卖不能买、燃料买入、静态提取即销毁、贡献值以它兑换 |

## 经济机制 <a href="#economic-mechanism" id="economic-mechanism"></a>

| 术语 | English | 定义 |
|---|---|---|
| 入金拆分 | Deposit split | 入金的 28% 买入 EXON 存为燃料，其余兑换 XO 进入质押 |
| 单次产出 | Per-epoch yield | 每 12 小时结算一次，0.2% – 0.6%；质押 10,000 U 一天 40 – 120 U |
| 期限加成 | Term bonus | 30 / 90 / 180 / 360 / 540 天对应基础 / +10% / +20% / +30% / +50%，只看期限不看金额 |
| 期满取回 | Maturity withdrawal | 30 天档期满可取回本金加收益，不收违约金；不取回自动续期 |
| 到账方式 | Settlement speed | 提取收益的三档：立即到账销毁 30%、30 天到账 20%、60 天到账 10% |
| 私募 | Private sale | 拿到 EXON 的第一条路：0.1 U，三档 1,000 / 5,000 / 10,000 U，共 11,500 份；第二条路是做动态 |
| 认购与质押配置 | Subscription-to-stake ratio | 私募期 3:1（质押金额取整数），上线后 1:1 |
| 线性释放 | Linear release | 上线当日起 1,095 天、2,190 次逐日释放 |
| 推广奖励 | Referral rewards | 最多 20 代、合计 76%，按下级每日静态产出计算，以 EXON 发放 |
| 等级极差 | Differential Matching Bonus | 领导奖金 V1 – V12 的发放方法：本人比例减去下级比例 |
| 永久销毁 | Burn | 提取静态收益时从燃料销毁的 EXON，永久退出流通 |
...
> XO 是价值锚，当前是质押本金代币：每 12 小时结算，静态收益以 XO 发放。
>
> EXON 是流通引擎，当前是核心价值代币：私募认购或做动态赚取，只能卖不能买，静态提取即销毁，贡献值以它兑换。
