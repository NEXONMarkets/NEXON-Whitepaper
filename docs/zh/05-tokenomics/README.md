---
description: "NEXON 现行经济机制：静态发 XO、动态发 EXON，每 12 小时结算，只进不出的 EXON 燃料，静态提取即销毁，贡献值以 EXON 兑换，私募 0.1 U 上线 1.0 U。"
icon: coins
---
...
| **当前机制** | 质押本金代币；静态收益以 XO 发放；在 NEX 二级市场自由交易 | 核心价值代币；私募 0.1 U 认购或做动态赚取，上线 1.0 U；只能卖、不能买，卖出手续费 5%；推广奖励与领导奖金以 EXON 发放；入金的 28% 买入存为燃料，提取静态收益时销毁；V3 起每日以 EXON 兑换贡献值 |
| **随入口开放** | 治理规则与更广的生态权益（Roadmap） | 交易、支付与兑换随五个入口逐个接入（Roadmap） |
...
{% hint style="success" %}
**燃料只进不出。** 里面的 EXON 不能提现、不能转出，只有一个去处：提取静态收益时销毁。质押越多，买入越多；提取越多，烧得越多。
{% endhint %}

## 经济生命周期 <a href="#the-economic-lifecycle" id="the-economic-lifecycle"></a>

**质押。** XO 质押每 12 小时结算一次，单次 0.2% – 0.6%；质押 10,000 U，一天 40 – 120 U。期限 30 / 90 / 180 / 360 / 540 天，加成基础 / +10% / +20% / +30% / +50%，只看期限不看金额。30 天档期满可取回本金加收益，不收违约金；不取回则自动续期 90 → 180 → 360 → 540 天。

**提取。** 静态收益以 XO 到账，随时可提取。到账方式三选一：立即到账销毁 30%、30 天到账销毁 20%、60 天到账销毁 10%，销毁的是燃料里等值的 EXON，永久退出流通。动态奖励以 EXON 到账，提取不销毁燃料。
