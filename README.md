...
| `onepage-cover.jpg` | Cover of the one-page overview |
| `onepage-01…12-*.svg` | The twelve figures of the one-page overview; several are reused by the cover page, the Staking & Reward Layer and the Token Economics chapter |
| `onepage-p1…p4-*.jpg` | Photo boards of the one-page overview (summit, team, awards and officials, certificates) |

`docs/overview/.gitbook/assets/` holds the Project Overview's own PNGs plus copies of the
one-page figures it reuses.

Figures are generated by `output/一文读懂/` in the parent workspace (`build.py` renders the
SVGs from `figs.py` / `charts.py`, `photos.py` builds the photo boards); every number in them
comes from `tokenomics.authority.json`. Changing a number means regenerating and copying into
**both** `docs/en/.gitbook/assets/` and `docs/zh/.gitbook/assets/` — `build_gitbook.py` does that.
